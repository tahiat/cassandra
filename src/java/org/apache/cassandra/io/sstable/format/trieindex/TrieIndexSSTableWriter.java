...
public class TrieIndexSSTableWriter extends SortedTableWriter
{
    ...
    public IRowIndexEntry endPartition() throws IOException
    {
        endPartitionMetadata();
        long trieRoot = partitionWriter.finish();
        IRowIndexEntry entry = TrieIndexEntry.create(currentStartPosition,
                                                    trieRoot,
    ...
    class IndexWriter extends AbstractTransactional implements Transactional
    {
        ...
        public long append(DecoratedKey key, IRowIndexEntry indexEntry) throws IOException
        {
            bf.add(key);
            long position;
