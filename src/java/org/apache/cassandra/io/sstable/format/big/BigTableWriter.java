...
public class BigTableWriter extends SortedTableWriter<BigFormatPartitionWriter>
{
    ...
    public SSTableReader openEarly()
    {
        ...
        try
        {
            ...
            try (CompressionMetadata compressionMetadata = compression ? ((CompressedSequentialWriter) dataWriter).open(boundary.dataLength) : null)
            {
    ...
    private SSTableReader openFinal(SSTableReader.OpenReason openReason)
    {
        ...
        try
        {
            ...
            dbuilder.withMmappedRegionsCache(mmappedRegionsCache);
            dbuilder.withChunkCache(chunkCache);
            try (CompressionMetadata compressionMetadata = compression ? ((CompressedSequentialWriter) dataWriter).open(0) : null)
            {
                dfile = dbuilder.bufferSize(dataBufferSize)
