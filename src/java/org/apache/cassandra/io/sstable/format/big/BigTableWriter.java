...
public class BigTableWriter extends SSTableWriter
{
    ...
    public SSTableReader openEarly()
    {
        ...
        try
        {
            ...
            dbuilder.withMmappedRegionsCache(mmappedRegionsCache);
            ...
            try (CompressionMetadata compressionMetadata = compression ? ((CompressedSequentialWriter) dataFile).open(boundary.dataLength) : null)
            {
                dfile = dbuilder.bufferSize(dataBufferSize)
                                .withLengthOverride(boundary.dataLength)
    ...
    private SSTableReader openFinal(SSTableReader.OpenReason openReason)
    {
        ...
        try
        {
            ...
            dbuilder.withMmappedRegionsCache(mmappedRegionsCache);
            dbuilder.withChunkCache(chunkCache);
            try (CompressionMetadata compressionMetadata = compression ? ((CompressedSequentialWriter) dataFile).open(0) : null)
            {
                dfile = dbuilder.bufferSize(dataBufferSize)
                                .withLengthOverride(NO_LENGTH_OVERRIDE)
                                .withCompressionMetadata(compressionMetadata)
                                .complete();
            }
            invalidateCacheAtBoundary(dfile);
    ...
    class IndexWriter extends AbstractTransactional implements Transactional
    {
        ...
        {
            indexFile = new SequentialWriter(new File(descriptor.filenameFor(Component.PRIMARY_INDEX)), ioOptions.writerOptions);
            builder = new FileHandle.Builder(descriptor.fileFor(Component.PRIMARY_INDEX)).mmapped(ioOptions.indexDiskAccessMode).withMmappedRegionsCache(mmappedRegionsCache);
        ...
        protected void doPrepare()
        {
            ...
            {
                new IndexSummaryComponent(indexSummary, first, last).save(descriptor, true);
            }
            catch (IOException ex)
            {
