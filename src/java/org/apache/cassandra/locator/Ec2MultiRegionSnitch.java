...
{
    private static final String PUBLIC_IP_QUERY = "/latest/meta-data/public-ipv4";
    private static final String PRIVATE_IP_QUERY = "/latest/meta-data/local-ipv4";
    ...
    @VisibleForTesting
    public Ec2MultiRegionSnitch(SnitchProperties props, Ec2MetadataServiceConnector connector) throws IOException, ConfigurationException
    {
        super(props, connector);
        InetAddress localPublicAddress = InetAddress.getByName(connector.awsApiCall(PUBLIC_IP_QUERY));
        logger.info("EC2Snitch using publicIP as identifier: {}", localPublicAddress);
        localPrivateAddress = connector.awsApiCall(PRIVATE_IP_QUERY);
